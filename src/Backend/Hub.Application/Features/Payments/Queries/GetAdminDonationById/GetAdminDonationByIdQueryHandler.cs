...
namespace Hub.Application.Features.Payments.Queries.GetAdminDonationById;

sealed class GetAdminDonationByIdQueryHandler(
    IPaymentsDbContext paymentsDbContext
) : IRequestHandler<GetAdminDonationByIdQuery, DonationResponse>
{
    public async Task<Result<DonationResponse>> Handle(
        GetAdminDonationByIdQuery query,
        CancellationToken cancellationToken)
    {
        var donation = await paymentsDbContext.Donations
            .FirstOrDefaultAsync(x => x.Id == query.DonationId, cancellationToken);
